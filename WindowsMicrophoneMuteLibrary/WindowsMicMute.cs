    ...
    public class WindowsMicMute
    {
        ...
        public bool MuteMic(int index, bool mute)
        {
            if (this.vistaMicMute != null)
                return this.vistaMicMute.MuteMic(index, mute);
            else
                return MixerNativeLibrary.MicInterface.MuteMic(index, mute);
        }

        public bool GetMute(int index)
        {
            if (this.vistaMicMute != null)
                return this.vistaMicMute.GetMute(index);
            else
                return MixerNativeLibrary.MicInterface.GetMute(index);
        }
    }
}
